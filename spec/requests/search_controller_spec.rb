...
describe SearchController do
  context "integration" do
    ...
    it 'performs the query with a topics type filter' do
      user = Fabricate(:user)
    ...
    context "user type filter" do
      let(:user) { Fabricate(:user) }

      it "does not return a searched-for user that does not belong to ICIJ project" do
        get "/search/query.json", params: {
      ...
      it "does return the searched-for user if it does belong to ICIJ project" do
        sign_in(user)
        ...
        get "/search/query.json", params: {
          term: other_user.username, type_filter: 'user'
        }
        ...
        expect(data['posts']).to be_blank
        expect(data['users'][0]['id']).to eq(other_user.id)
      end

      it "does not return the searched-for user if neither the signed-in user nore the searched-for user belong to ICIJ project" do
        sign_in(user)
        other_user = Fabricate(:user)
  ...
  describe "searching by query" do
    context "logs" do
