...
describe Search do
  ...
  context 'users when neither party is a member of the ICIJ project' do
    let!(:user) { Fabricate(:user) }
    let(:result) { Search.execute('bruce', type_filter: 'user') }

    it 'returns no users' do
      expect(result.users.length).to eq(0)
      expect(result.users[0]).to be_blank
    end
    ...
    context 'hiding user profiles' do
      ...
      it 'returns no users for logged in users' do
  ...
  context 'type_filter' do
    ...
    context 'user filter when neither party is part of ICIJ project' do
